...
function project() {
  const root = mkdtempSync(path.join(tmpdir(), "specocd-cli-"));
  roots.push(root);
  run(root, ["init"]);
  return root;
}
...
describe("cli exit codes", () => {
  ...
  test("verify exits 0 once the change is complete", () => {
    ...
    const dir = path.join(root, ".specocd", "changes", "clean");
    writeFileSync(
  ...
  test("a second session claiming a live task exits 2", () => {
    ...
    assert.equal(run(root, ["claim", "contested", "T1"], { SPECOCD_SESSION_ID: "a" }).code, 0);
    const conflict = run(root, ["claim", "contested", "T1"], { SPECOCD_SESSION_ID: "b" });
  ...
  test("operating outside an initialized project exits 1 with guidance", () => {
    const bare = mkdtempSync(path.join(tmpdir(), "specocd-bare-"));
    roots.push(bare);
    const result = run(bare, ["status"]);
    assert.equal(result.code, 1);
    assert.match(result.stderr, /specocd init/);
  });
...
describe("cli output", () => {
  test("status --json is parseable and reflects claims", () => {
    ...
    run(root, ["claim", "jsonic", "T1"], { SPECOCD_AGENT: "cursor", SPECOCD_SESSION_ID: "s1" });
  ...
  test("claiming surfaces prior decisions so context carries across agents", () => {
    ...
    run(root, ["claim", "handoff", "T1"], { SPECOCD_AGENT: "claude-code", SPECOCD_SESSION_ID: "s1" });
    run(root, ["log", "handoff", "--task", "T1", "--type", "decision", "--message", "picked approach X"],
      { SPECOCD_AGENT: "claude-code", SPECOCD_SESSION_ID: "s1" });
    run(root, ["release", "handoff", "T1", "--status", "released"], { SPECOCD_SESSION_ID: "s1" });

    const pickup = run(root, ["claim", "handoff", "T1"], { SPECOCD_AGENT: "cursor", SPECOCD_SESSION_ID: "s2" });
