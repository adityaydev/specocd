...
function prompt(name: string, description: string, body: string): GeneratedFile {
  return {
    path: path.join(".github", "prompts", `specocd-${name}.prompt.md`),
    mode: "replace",
...
Run \`specocd status\` first, claim the task you intend to work, then implement only what
that task covers.
...
${WORKFLOW}`,
      ...
      prompt(
        ...
        `Run \`specocd verify <change>\`, then check each WHEN/THEN requirement against the
