...
### FR1 — Init & Adoption
- FR1.1: A single init command scaffolds a `.specdd/` (or similar) directory into any existing repo without touching existing files.
...
|---|---|---|---|
| 1 | Claim/lock file format | Separate `claims.yaml` per change (`.specdd/changes/<name>/claims.yaml`), scoped to the change, not global | Keeps coordination metadata out of task content (avoids diff noise / merge conflicts on files agents read for requirements); fast to scan without opening every task file |
...
|---|---|---|
| `specdd init` | Scaffold `.specdd/`, detect installed agent tooling, generate bindings | FR1 |
| `specdd propose <name>` | Create a new change folder with templates | FR4.1 |
| `specdd claim <change> <task_id>` | Claim a task (writes/updates `claims.yaml`) | FR3.1 |
| `specdd release <change> <task_id> [--status completed\|abandoned]` | End a claim cleanly | FR3.6 |
| `specdd log <change> --task <task_id> --type <decision\|blocker\|status> --message "..."` | Append a structured event | FR3.2 |
| `specdd status [<change>]` | Show claims, staleness, flagged conflicts across the project or one change | FR3.3, FR3.6 |
| `specdd verify <change>` | Check implementation against WHEN/THEN acceptance criteria | FR4.2 |
| `specdd archive <change>` | Fold change into baseline `specs/`, move to `archive/`; enforces `require_approval` if set | FR2.3, decision #6 |
| `specdd digest <change>` | Force-regenerate `digest.md` (also auto-triggered on cap breach) | FR2.4 |
| `specdd bindings sync` | Refresh generated per-agent binding files after a framework update | FR5.3 |
...
- **Phase 4 — Polish**: `specdd status` conflict/staleness reporting UX, documentation, packaging for npm publish.
